...
function BOMPage({ socket: providedSocket }) {
  ...
  const fetchProductsAndMaterials = useCallback(async () => {
    try {
      ...
      console.log('Products fetched:', productData); // Debug
      ...
      console.log('Materials fetched:', materialData); // Debug
  ...
  useEffect(() => {
    ...
    socket.on('bom:deleted', handleBomDeleted);
    ...
    return () => {
      ...
      socket.off('bom:deleted', handleBomDeleted);
  ...
  const handleDelete = useCallback(
    async (bomId) => {
      if (!window.confirm(`Are you sure you want to delete BOM #${bomId}?`)) return;

      try {
        const token = localStorage.getItem('token');
        const response = await fetch(`${BASE_URL}/api/bom/${bomId}`, {
          method: 'DELETE',
          headers: {
