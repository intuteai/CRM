...
// Parse current user from localStorage (stored as JSON "user")
...
function DesignEnquiryPage({ socket: providedSocket }) {
  ...
  const fetchEnquiryDetail = useCallback(
    async (enquiryId) => {
      try {
        ...
        // Mark activities as read (fire and forget)
  ...
  // FILTERED (Design sees only own assigned enquiries if user info present)
    ...
    <div className="max-w-7xl mx-auto">
      <div>
        ...
        {actionsMenuState.isOpen && actionsMenuState.enquiry && (
          ...
          >
            <div
              className="absolute z-50 w-52 bg-white border border-gray-200 rounded-xl shadow-lg py-1 pointer-events-auto"
              style={{
                left: Math.min(Math.max(actionsMenuState.x, 112), window.innerWidth - 112),
                top: Math.max(8, Math.min(actionsMenuState.y, window.innerHeight - 300)),
