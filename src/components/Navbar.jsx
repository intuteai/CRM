...
function Navbar({ userRole, userName, token, setUserRole, setShowLogin, handleLogout }) {
  ...
  const AdminLinks = () => (
    <div className="flex flex-col lg:flex-row lg:space-x-6 space-y-3 lg:space-y-0">
      <Link
        to="/admin-dashboard"
        className="text-gray-200 text-base lg:text-lg font-medium hover:text-amber-300 transition-all duration-300 transform hover:scale-105 hover:shadow-md px-4 py-2 rounded-lg bg-gray-700 bg-opacity-50 focus:outline-none focus:ring-2 focus:ring-amber-300"
  ...
  const UserControls = () => (
    <div className="flex flex-col lg:flex-row lg:items-center lg:space-x-6 space-y-3 lg:space-y-0">
      ...
      <span className="text-gray-200 text-base lg:text-lg font-medium flex items-center break-words">
        <Clock className="mr-2 w-5 h-5 flex-shrink-0" aria-hidden="true" />
        <span className="flex-wrap">Hello, {userName} | {currentTime} (IST)</span>
      </span>
      <button
        onClick={() => { handleLogoutClick(); setIsMobileMenuOpen(false); }}
        className="bg-amber-400 text-gray-900 text-base lg:text-lg font-semibold px-6 py-3 rounded-lg shadow-md hover:bg-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-300 transition-all duration-300 transform hover:scale-105 flex items-center"
        aria-label="Logout"
      >
  ...
  return (
    <nav className="bg-gradient-to-r from-gray-800 to-gray-900 p-6 shadow-lg">
      <div className="container mx-auto">
        ...
        <div className="hidden lg:flex justify-between items-center">
          ...
          >
            <span className="relative group">
              intute.ai
              <span className="absolute -bottom-1 left-0 w-0 h-1 bg-amber-400 group-hover:w-full transition-all duration-300"></span>
            </span>
          </Link>
        ...
        {/* Mobile Menu */}
        {isMobileMenuOpen && (
          <div className="lg:hidden mt-6 space-y-6 py-6 border-t border-gray-700">
            {userRole === 'admin' && <AdminLinks />}
            {userRole === 'customer' && <CustomerLinks />}
            {userRole && <UserControls />}
          </div>
