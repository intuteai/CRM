...
function Navbar({ userRole, userName, token, setUserRole, setShowLogin, handleLogout }) {
  ...
  const AdminLinks = () => (
    <div className="flex flex-col lg:flex-row lg:space-x-4 space-y-2 lg:space-y-0">
      <Link
        to="/admin-dashboard"
        className="text-gray-200 text-sm font-medium hover:text-amber-300 transition-all duration-300 transform hover:scale-105 hover:shadow-md px-3 py-1 rounded-lg bg-gray-700 bg-opacity-50 focus:outline-none focus:ring-2 focus:ring-amber-300"
  ...
  const UserControls = () => (
    <div className="flex flex-col lg:flex-row lg:items-center lg:space-x-4 space-y-2 lg:space-y-0">
      <Link
        ...
        onClick={() => setIsMobileMenuOpen(false)}
      >
        <User className="inline-block mr-1 w-4 h-4" aria-hidden="true" />
        Edit Profile
      </Link>
      <span className="text-gray-200 text-sm font-medium flex items-center break-words">
        <Clock className="mr-1 w-4 h-4 flex-shrink-0" aria-hidden="true" />
        <span className="flex-wrap">Hello, {userName} | {currentTime} (IST)</span>
      </span>
      <button
        onClick={() => { handleLogoutClick(); setIsMobileMenuOpen(false); }}
        className="bg-amber-400 text-gray-900 text-sm font-semibold px-4 py-2 rounded-lg shadow-md hover:bg-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-300 transition-all duration-300 transform hover:scale-105 flex items-center"
        aria-label="Logout"
      >
        <LogOut className="mr-1 w-4 h-4" aria-hidden="true" />
        Logout
      </button>
    </div>
  );
  ...
  return (
    <nav className="bg-gradient-to-r from-gray-800 to-gray-900 p-4 shadow-lg">
      <div className="container mx-auto">
        ...
        {/* Mobile Menu */}
        {isMobileMenuOpen && (
          <div className="lg:hidden mt-4 space-y-4 py-4 border-t border-gray-700">
            {userRole === 'admin' && <AdminLinks />}
            {userRole === 'customer' && <CustomerLinks />}
            {userRole && <UserControls />}
          </div>
