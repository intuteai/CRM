    ...
    try {
      ...
      if (isMounted) {
        const normalizedData = data.map(item => ({
          ...
          created_at: item.created_at ? new Date(item.created_at).toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' }) : '',
        }));
...
function StoreInventoryPage({ userRole }) {
  ...
  const sortedInventory = useMemo(() => {
    ...
    if (sortConfig.key) {
      sortableInventory.sort((a, b) => {
        ...
        if (sortConfig.key === 'stock_quantity' || sortConfig.key === 'price') {
  ...
  const exportToExcel = useCallback(() => {
    const data = filteredInventory.map(item => ({
      ...
      'Created At (IST)': item.created_at || 'N/A',
    }));
    ...
    XLSX.writeFile(workbook, 'Store_Inventory.xlsx');
    toast.success('Store Inventory exported to Excel!', { autoClose: 2000 });
  }, [filteredInventory]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 to-gray-100 p-8">
      <h1 className="text-4xl font-bold text-gray-800 mb-10 text-center">Store Inventory</h1>
      <div className="max-w-7xl mx-auto">
        <div className="flex mb-8 gap-6 flex-wrap">
          <div className="relative flex-grow">
            ...
            <input
              id="search-input"
              type="text"
              placeholder="Search by Product ID, Name, Product Code, or Description..."
              value={searchInput}
        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-x-auto">
          <table className="w-full text-left" role="grid" aria-label="Store inventory table" ref={tableRef} tabIndex={0}>
            ...
            <tbody>
              {filteredInventory.map(item => (
                <tr key={item.product_id} className="border-t hover:bg-amber-50" role="row">
                  ...
                  <td className="py-4 px-3">{item.price.toFixed(2)}</td>
                  <td className="py-4 px-3">{item.created_at}</td>
                  <td className="py-4 px-3">
                    <button
...
const CreateItemForm = ({ onSubmit, onClose }) => {
  ...
  const validateField = (name, value) => {
    ...
    if (name === 'stock_quantity' && value < 0) return 'Stock Quantity cannot be negative';
