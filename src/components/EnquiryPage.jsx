...
function EnquiryPage({ socket: providedSocket }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const body = {
        ...
        lead: newEnquiry.lead, // 🔥 send lead
  ...
  const filteredEnquiries = useMemo(() => {
    ...
    return enquiries.filter((item) => {
      ...
      const matchesLead =
        leadFilter === 'all' ||
        leadValue === leadFilter.toLowerCase();
      ...
      const matchesTag =
        ...
        (Array.isArray(item.tags) &&
          item.tags.some((t) =>
            String(t || '').toLowerCase().includes(tagFilterLower)
          ));
              ...
              <tr className="bg-gradient-to-r from-amber-300 via-amber-200 to-amber-100 text-gray-800">
                {[
                  ...
                  <th
                    ...
                    className={`px-6 md:px-8 py-4 text-sm md:text-base font-bold ${
                      key !== 'actions'
                        ...
                        <ArrowDownUp
                          ...
                          className={`ml-2 text-gray-700 ${
                            sortConfig.key === key
                              ...
                              : 'opacity-60'
                          } hover:text-amber-800 transition-colors duration-200`}
                        />
                      )}
                    </div>
                  </th>
              ...
              {sortedEnquiries.map((enquiry, index) => {
                ...
                return (
                  <tr
                    ...
                    onDoubleClick={() =>
                      fetchEnquiryDetail(enquiry.enquiry_id)
                    }
                  >
                    ...
                    <td
                      className={`px-6 md:px-8 py-4 text-gray-600 font-semibold ${
                  ...
                  <div className="mb-4 border-b pb-4 flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        Contact:{' '}
                        {detailEnquiry.contact_person ||
                          'Not specified'}{' '}
                        • Phone:{' '}
                        {detailEnquiry.phone_no || 'Not specified'}
                      </p>
                    </div>
                    ...
                    <div className="rounded-2xl border border-amber-100 bg-white/70 px-3 py-2 shadow-sm">
                      ...
                      <p className="text-[11px] text-gray-500 mt-1">
                        ...
                        <span className="font-medium">
                          {detailEnquiry.assigned_to_name ||
                            'Not assigned'}
                        </span>
                      </p>
                    </div>
