...
const useFetchInventory = ({ limit, offset }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Use the /available endpoint to get reserved/available quantities
...
function ProductionInventoryPage() {
  ...
  // NEW: Quantity and Reserve modals
  ...
  const sortedInventory = useMemo(() => {
    ...
    if (sortConfig.key) {
      sortableInventory.sort((a, b) => {
        ...
        if (sortConfig.key === 'price' || sortConfig.key === 'stock_quantity' || sortConfig.key === 'returnable_qty' || sortConfig.key === 'available_quantity') {
  ...
  const handleCreateItem = useCallback(
    async ({ product_name, stock_quantity, price, description, product_code, returnable_qty = 0 }) => {
      const token = localStorage.getItem('token');
      try {
        if (!token) throw new Error('Authentication token missing.');
        const backendUrl = import.meta.env.VITE_BACKEND_URL || 'http://localhost:5000';
    ...
    <div className="min-h-screen bg-gradient-to-br from-amber-50 to-gray-100 p-8">
      ...
      {showCreateForm && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-70 flex items-center justify-center z-50 p-4">
          <div
            className="bg-white p-8 rounded-2xl shadow-xl w-[500px] max-h-[90vh] overflow-y-auto relative"
            role="dialog"
...
/* ========= QuantityBreakdownModal Component ========= */
...
const CreateItemForm = ({ onSubmit, onClose }) => {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'stock_quantity' || name === 'returnable_qty' || name === 'price') {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await onSubmit({
        ...
        returnable_qty: isNaN(parsedReturnable) ? 0 : parsedReturnable,
      });
    } finally {
      setIsSubmitting(false);
    }
  ...
  return (
    <form className="space-y-4" onSubmit={e => e.preventDefault()}>
      ...
      <div>
        <label htmlFor="create-returnable-qty" className="text-gray-700 font-medium">
          Returnable Qty
        </label>
        <input
...
const EditItemForm = ({ item, onSubmit, onClose }) => {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'stock_quantity' || name === 'price' || name === 'returnable_qty') {
      setFormData(prev => ({ ...prev, [name]: value }));
      setErrors(prev => ({ ...prev, [name]: '' }));
      return;
    }
  ...
  return (
    <form className="space-y-4" onSubmit={e => e.preventDefault()}>
      ...
      <div>
        <label htmlFor="edit-returnable-qty" className="text-gray-700 font-medium">
          Returnable Qty
        </label>
        <input
