  ...
  const ActionsDropdown = ({ item, onEdit }) => {
    ...
    return (
      <div ref={dropdownRef} className="relative">
        <button onClick={() => setIsOpen(!isOpen)} className="p-2 hover:bg-gray-100 rounded-full focus:outline-none focus:ring-2 focus:ring-gold-400" aria-label={`Actions for item ${item.product_name}`} aria-haspopup="true" aria-expanded={isOpen}>
        ...
        {isOpen && (
          <div className="absolute right-0 z-10 mt-2 w-56 bg-white shadow-lg rounded-lg ring-1 ring-black ring-opacity-5">
            ...
            <button onClick={() => { setReserveProduct(item); setShowReserveModal(true); setIsOpen(false); }} className="flex items-center w-full px-4 py-2 text-sm text-blue-700 hover:bg-blue-50 focus:outline-none focus:bg-blue-50">
    ...
    <div className="max-w-7xl mx-auto space-y-4">
      ...
      {showCreateForm && (
        <div className="fixed inset-0 bg-navy-900/50 flex items-center justify-center z-50 p-4">
          <div className="bg-white p-6 rounded-xl shadow-2xl w-[560px] max-w-full max-h-[90vh] overflow-y-auto relative" role="dialog" aria-labelledby="create-form-title">
      ...
      {showEditForm && selectedItem && (
        <div className="fixed inset-0 bg-navy-900/50 flex items-center justify-center z-50 p-4">
          <div className="bg-white p-6 rounded-xl shadow-2xl w-[560px] max-w-full max-h-[90vh] overflow-y-auto relative" role="dialog" aria-labelledby="edit-form-title">
      ...
      {viewingItem && (
        <div className="fixed inset-0 bg-navy-900/50 flex items-center justify-center z-50 p-4">
          <div className="bg-white p-6 rounded-xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto relative" role="dialog" aria-labelledby="item-details-title">
      ...
      {/* QR Code Modal */}
      {showBarcodeModal && (
        <div className="fixed inset-0 bg-navy-900/50 flex items-center justify-center z-50 overflow-auto">
          <div className="bg-white p-6 rounded-xl shadow-2xl w-[90%] max-w-[500px] max-h-[90vh] relative flex flex-col" role="dialog" aria-labelledby="qrcode-modal-title">
...
const QuantityBreakdownModal = ({ product, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-navy-900/50 flex items-center justify-center z-50">
      <div className="bg-white w-[92%] sm:w-[90%] max-w-[900px] rounded-xl shadow-2xl p-4 sm:p-6 relative max-h-[90vh] overflow-y-auto">
        ...
        {loading ? <p className="text-gray-500 py-4">Loading holds…</p> : holds.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full border rounded-lg">
              ...
              <tbody>
                {holds.map(h => (
                  <tr key={h.hold_id} className="border-t hover:bg-navy-50">
                    <td className="p-3">{h.reason}</td>
                    <td className="p-3"><span className="px-2 py-1 bg-gold-400/25 text-gold-600 rounded-full text-sm font-semibold">{h.quantity}</span></td>
        ...
        <div className="mt-6 flex justify-end">
          <button onClick={onClose} className="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg font-medium hover:bg-gray-200 transition-colors">Close</button>
        </div>
      </div>
    </div>
  );
...
const Stat = ({ label, value, highlight }) => (
  <div className={`p-4 rounded-lg text-center ${highlight ? 'bg-gold-400/25 border border-gold-400/40' : 'bg-navy-50'}`}>
    <div className="text-sm text-gray-600 mb-1">{label}</div>
    <div className="text-2xl font-bold text-navy-800">{value ?? 0}</div>
  </div>
);
...
const ReserveStockModal = ({ product, onClose, onReserved }) => {
  ...
  return (
    <div className="fixed inset-0 bg-navy-900/50 flex items-center justify-center z-50 p-4">
      <div className="bg-white p-6 rounded-xl shadow-2xl w-[480px] max-w-full max-h-[90vh] overflow-y-auto relative">
        ...
        <h3 className="font-display text-xl font-bold text-navy-800 mb-3 pr-8 flex items-center"><Lock size={20} className="mr-2 text-blue-600" />Reserve Stock — {product.product_name}</h3>
        ...
        <div className="space-y-4">
          <div><label className="text-sm font-medium text-navy-800 block mb-1">Quantity to Reserve</label><input type="number" min={1} max={product.available_quantity} value={quantity} onChange={(e) => setQuantity(e.target.value)} className="w-full p-2.5 border border-navy-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-400" disabled={isSubmitting} /></div>
          <div><label className="text-sm font-medium text-navy-800 block mb-1">Reason <span className="text-red-500">*</span></label><input type="text" value={reason} onChange={(e) => setReason(e.target.value)} placeholder="e.g., Reserved for order #123" className="w-full p-2.5 border border-navy-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-400" disabled={isSubmitting} /></div>
          <div><label className="text-sm font-medium text-navy-800 block mb-1">Reference Type (optional)</label>
            <select value={referenceType} onChange={(e) => setReferenceType(e.target.value)} className="w-full p-2.5 border border-navy-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-400" disabled={isSubmitting}>
              <option value="">-- Select --</option><option value="ORDER">Order</option><option value="QA">Quality Assurance</option><option value="MANUAL">Manual Hold</option>
            </select>
          </div>
          {referenceType && <div><label className="text-sm font-medium text-navy-800 block mb-1">Reference Value</label><input type="text" value={referenceValue} onChange={(e) => setReferenceValue(e.target.value)} placeholder={referenceType === 'ORDER' ? 'Order ID' : 'Reference value'} className="w-full p-2.5 border border-navy-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-400" disabled={isSubmitting} /></div>}
        </div>
        <div className="flex space-x-3 justify-end mt-6">
          <button onClick={onClose} className="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg font-medium hover:bg-gray-200 transition-colors" disabled={isSubmitting}>Cancel</button>
          <button onClick={handleReserve} className="px-4 py-2 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition-colors flex items-center" disabled={isSubmitting}>
  ...
  return (
    <div className="fixed inset-0 bg-navy-900/50 flex items-center justify-center z-50 p-4">
      <div className="bg-white p-6 rounded-xl shadow-2xl w-[420px] max-w-full max-h-[90vh] overflow-y-auto relative">
        ...
        <label className="text-sm font-medium text-navy-800">Quantity to accept</label>
        <input type="number" min={1} max={product.returnable_qty} value={qty} onChange={(e) => setQty(e.target.value)} className="w-full p-2.5 border border-navy-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-400 mb-3" />
        <div className="flex space-x-3 justify-end">
          <button onClick={onClose} className="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg font-medium hover:bg-gray-200 transition-colors" disabled={isSubmitting}>Cancel</button>
          <button onClick={handleAccept} className="px-4 py-2 bg-navy-800 text-white rounded-lg font-medium hover:bg-navy-700 transition-colors flex items-center" disabled={isSubmitting}>
  ...
  return (
    <form className="space-y-4" onSubmit={e => e.preventDefault()}>
      ...
      <div>
        ...
        <div className="relative" ref={partDropdownRef}>
          <input type="text" value={partSearch}
            ...
            className="w-full p-2.5 pl-8 border border-navy-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-400" disabled={isSubmitting} />
          ...
          {showPartDropdown && (
            <div className="absolute z-20 mt-1 w-full max-h-60 overflow-y-auto bg-white border rounded-lg shadow-lg">
              ...
              {!isPartLoading && filteredParts.map(part => (
                <button key={part.id} type="button" onClick={() => handlePartSelect(part)} className="w-full text-left px-3 py-2 text-sm hover:bg-navy-50">
      ...
      <div>
        ...
        <input id="create-product-name" type="text" name="product_name" value={formData.product_name} onChange={handleChange} className="w-full p-2.5 border border-navy-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-400" disabled={isSubmitting} />
      ...
      <div>
        ...
        <textarea id="create-description" name="description" value={formData.description} onChange={handleChange} className="w-full p-2.5 border border-navy-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-400" disabled={isSubmitting} />
      </div>
      <div>
        <label htmlFor="create-stock-quantity" className="text-gray-700 font-medium">Stock Quantity</label>
        <input id="create-stock-quantity" type="number" name="stock_quantity" value={formData.stock_quantity} onChange={handleChange} className="w-full p-2.5 border border-navy-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-400" disabled={isSubmitting} />
      ...
      <div>
        ...
        <input id="create-returnable-qty" type="number" name="returnable_qty" value={formData.returnable_qty} onChange={handleChange} min={0} className="w-full p-2.5 border border-navy-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-400" disabled={isSubmitting} />
      ...
      <div>
        ...
        <input id="create-price" type="number" name="price" value={formData.price} onChange={handleChange} step="0.01" className="w-full p-2.5 border border-navy-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-400" disabled={isSubmitting} />
        {errors.price && <p className="text-red-600 text-sm mt-1">{errors.price}</p>}
      </div>

      <div className="flex justify-end space-x-4">
        <button type="button" onClick={onClose} className="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg font-medium hover:bg-gray-200 transition-colors" disabled={isSubmitting}>Cancel</button>
        <button type="button" onClick={handleSave} className="px-4 py-2 bg-gold-500 text-navy-900 rounded-lg font-semibold hover:bg-gold-400 transition-colors flex items-center" disabled={isSubmitting}>
  ...
  return (
    <form className="space-y-4" onSubmit={e => e.preventDefault()}>
      <div>
        ...
        <input id="edit-product-name" type="text" name="product_name" value={formData.product_name} onChange={handleChange} className="w-full p-2.5 border border-navy-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-400" disabled={isSubmitting} />
        {errors.product_name && <p className="text-red-600 text-sm mt-1">{errors.product_name}</p>}
      </div>
      ...
      <div>
        ...
        <textarea id="edit-description" name="description" value={formData.description} onChange={handleChange} className="w-full p-2.5 border border-navy-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-400" disabled={isSubmitting} />
      </div>
      <div>
        <label htmlFor="edit-stock-quantity" className="text-gray-700 font-medium">Stock Quantity</label>
        <input id="edit-stock-quantity" type="number" name="stock_quantity" value={formData.stock_quantity} onChange={handleChange} className="w-full p-2.5 border border-navy-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-400" disabled={isSubmitting} />
        {errors.stock_quantity && <p className="text-red-600 text-sm mt-1">{errors.stock_quantity}</p>}
      </div>
      <div>
        <label htmlFor="edit-returnable-qty" className="text-gray-700 font-medium">Returnable Qty</label>
        <input id="edit-returnable-qty" type="number" name="returnable_qty" value={formData.returnable_qty} onChange={handleChange} min={0} className="w-full p-2.5 border border-navy-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-400" disabled={isSubmitting} />
        {errors.returnable_qty && <p className="text-red-600 text-sm mt-1">{errors.returnable_qty}</p>}
      </div>
      <div>
        <label htmlFor="edit-price" className="text-gray-700 font-medium">Price (₹)</label>
        <input id="edit-price" type="number" name="price" value={formData.price} onChange={handleChange} step="0.01" className="w-full p-2.5 border border-navy-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gold-400" disabled={isSubmitting} />
        {errors.price && <p className="text-red-600 text-sm mt-1">{errors.price}</p>}
      </div>

      <div className="flex justify-end space-x-4">
        <button type="button" onClick={onClose} className="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg font-medium hover:bg-gray-200 transition-colors" disabled={isSubmitting}>Cancel</button>
        <button type="button" onClick={handleSave} className="px-4 py-2 bg-navy-800 text-white rounded-lg font-medium hover:bg-navy-700 transition-colors flex items-center" disabled={isSubmitting}>
