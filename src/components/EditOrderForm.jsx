...
function EditOrderForm({ order, availableProducts, onClose, onSubmit, validateOrderItems, getAvailableStock, formatDate }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editedOrder.targetDeliveryDate) {
      const selectedDate = new Date(editedOrder.targetDeliveryDate + 'T00:00:00'); // Treat as local date
    ...
    try {
      ...
      const payload = {
        ...
        targetDeliveryDate: editedOrder.targetDeliveryDate,
                ...
                >
                  ...
                  {availableProducts.filter(p => p && typeof p.product_id !== 'undefined').map(product => (
                    <option
                      ...
                      disabled={getAvailableStock(product.product_id, order.id) <= 0 && item.product_id !== product.product_id}
                    >
