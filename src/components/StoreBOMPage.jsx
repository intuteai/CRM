...
//   const [formData, setFormData] = useState({
//     productName: '',
//     materials: [{ materialName: '', quantityPerUnit: '' }],
//   });

//   // You may not need products/materials arrays after this but keeping in case for future
//   const [products, setProducts] = useState([]);
//   const [materials, setMaterials] = useState([]);
//   const [sortConfig, setSortConfig] = useState({ key: 'bomId', direction: 'desc' });
//   const [page, setPage] = useState(0);
...
//   const fetchProductsAndMaterials = useCallback(async () => {
//     try {
//       // Fetching code can stay if you want to show suggestions/autocomplete in future
//       setIsDataLoaded(true);
//     } catch (err) {
//       toast.error(`Failed to load products or materials: ${err.message}`, { autoClose: 3000 });
//       setIsDataLoaded(true);
...
//     setFormData((prev) => ({
//       ...prev,
//       materials: [...prev.materials, { materialName: '', quantityPerUnit: '' }],
//     }));
//   }, []);
...
//           throw new Error(
//             errorText ||
//             `${modalMode === 'create' ? 'Create' : 'Update'} failed with status: ${response.status}`
//           );
//         }
...
//                           </thead>
//                           <tbody>
//                             {bom.materials.map((material, i) => (
//                               <tr key={material.bomMaterialId || i} className="border-t">
//                                 <td className="py-3 px-3 text-gray-600 text-sm">{material.materialName || 'N/A'}</td>
//                                 <td className="py-3 px-3 text-gray-600 text-sm">{material.quantityPerUnit}</td>
//                               </tr>
//                             ))}
