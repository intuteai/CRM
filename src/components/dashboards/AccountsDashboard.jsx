...
import {
  FileText, MapPin, ShoppingCart, Truck
} from 'lucide-react';

function AccountsDashboard({ socket }) {
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 to-gray-100 p-6">
      ...
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <Section title="Finance Management">
          <DashboardCard to="/customer-invoices" icon={<FileText />} title="Customer Invoices" desc="Manage customer billing and payments" />
          <DashboardCard to="/purchase-invoices" icon={<ShoppingCart />} title="Purchase Invoices" desc="Track and manage purchase expenses" />
        </Section>

        {/* Dispatch and Sales */}
        <Section title="Dispatch & Sales">
          <DashboardCard to="/dispatch-tracking" icon={<MapPin />} title="Dispatch Tracking" desc="Track dispatch status of orders" />
          <DashboardCard to="/orders" icon={<Truck />} title="Orders" desc="Access and manage orders" />
        </Section>
      </div>
    </div>
  );
